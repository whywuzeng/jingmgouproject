  ...
  implements ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnTouchModeChangeListener
{
  ...
  {
    this(paramContext, paramAttributeSet, 2130772030);
  }
  ...
  public PLA_AbsListView(Context paramContext, AttributeSet paramAttributeSet, int paramInt)
  {
    ...
    this.mDrawSelectorOnTop = paramContext.getBoolean(1, false);
    setStackFromBottom(paramContext.getBoolean(2, false));
    setScrollingCacheEnabled(paramContext.getBoolean(3, true));
    setTranscriptMode(paramContext.getInt(5, 0));
    setCacheColorHint(paramContext.getColor(6, 0));
    setSmoothScrollbarEnabled(paramContext.getBoolean(8, true));
    paramContext.recycle();
  }
      ...
      this.mClearScrollingCache = new Runnable()
      {
        public void run()
        {
          if (PLA_AbsListView.this.mCachingStarted)
          {
            ...
            if ((PLA_AbsListView.this.getPersistentDrawingCache() & 0x2) == 0)
  ...
  private boolean startScrollIfNeeded(int paramInt1, int paramInt2)
  {
    if (Math.abs(paramInt1) > this.mTouchSlop)
    {
      createScrollingCache();
      this.mTouchMode = 3;
      this.mMotionCorrection = paramInt1;
      setPressed(false);
  ...
  {
    return new PLA_AdapterView.AdapterContextMenuInfo(paramView, paramInt, paramLong);
  }
  ...
  public boolean onInterceptTouchEvent(MotionEvent paramMotionEvent)
  {
    ...
    while (true)
    {
      ...
      if (i == 4)
      {
        System.out.println("--->return1 true");
        return true;
        ...
        case 0:
          ...
          if (startScrollIfNeeded(i - this.mMotionY, j - this.mMotionX))
          {
                  ...
                  postDelayed(new Runnable()
                  {
                    public void run()
                    {
                      ...
                      if (!PLA_AbsListView.this.mDataChanged)
                        PLA_AbsListView.this.post(localPerformClick);
    ...
    implements Runnable
  {
    ...
    public void run()
    {
      ...
      {
        PLA_AbsListView.this.mTouchMode = 1;
        localObject = PLA_AbsListView.this.getChildAt(PLA_AbsListView.this.mMotionPosition - PLA_AbsListView.this.mFirstPosition);
        if ((localObject != null) && (!((View)localObject).hasFocusable()))
        {
          ...
          PLA_AbsListView.this.positionSelector((View)localObject);
          PLA_AbsListView.this.setPressed(true);
          int i = ViewConfiguration.getLongPressTimeout();
          bool = PLA_AbsListView.this.isLongClickable();
          if (PLA_AbsListView.this.mSelector != null)
          {
            localObject = PLA_AbsListView.this.mSelector.getCurrent();
      ...
      label177: PLA_AbsListView.this.mTouchMode = 2;
    }
  }
    ...
    implements Runnable
  {
    ...
    private final Scroller mScroller = new Scroller(PLA_AbsListView.this.getContext());
    ...
    public void run()
    {
      ...
      for (i = Math.min(PLA_AbsListView.this.getHeight() - PLA_AbsListView.this.getPaddingBottom() - PLA_AbsListView.this.getPaddingTop() - 1, i); ; i = Math.max(-(PLA_AbsListView.this.getHeight() - PLA_AbsListView.this.getPaddingBottom() - PLA_AbsListView.this.getPaddingTop() - 1), i))
      {
        boolean bool2 = PLA_AbsListView.this.trackMotionScroll(i, i);
        if ((!bool1) || (bool2))
          break;
        PLA_AbsListView.this.invalidate();
        this.mLastFlingY = j;
        ...
        return;
        int k = PLA_AbsListView.this.getChildCount();
        PLA_AbsListView.this.mMotionPosition = (PLA_AbsListView.this.mFirstPosition + (k - 1));
    ...
    {
      int i = PLA_AbsListView.this.modifyFlingInitialVelocity(paramInt);
      if (i < 0);
  ...
  public static abstract interface OnScrollListener
  {
    ...
    public abstract void onScroll(PLA_AbsListView paramPLA_AbsListView, int paramInt1, int paramInt2, int paramInt3);

    public abstract void onScrollStateChanged(PLA_AbsListView paramPLA_AbsListView, int paramInt);
  }

  private class PerformClick extends PLA_AbsListView.WindowRunnnable
    implements Runnable
  {
    View mChild;
    ...
    public void run()
    {
      ...
      {
        return;
        localListAdapter = PLA_AbsListView.this.mAdapter;
        i = this.mClickMotionPosition;
      }
      while ((localListAdapter == null) || (PLA_AbsListView.this.mItemCount <= 0) || (i == -1) || (i >= localListAdapter.getCount()) || (!sameWindow()));
      PLA_AbsListView.this.performItemClick(this.mChild, i, localListAdapter.getItemId(i));
    }
  }
    ...
    implements Runnable
  {
    ...
    private final int mExtraScroll = ViewConfiguration.get(PLA_AbsListView.this.getContext()).getScaledFadingEdgeLength();
    ...
    public void run()
    {
      int j = PLA_AbsListView.this.getHeight();
      int k = PLA_AbsListView.this.mFirstPosition;
      switch (this.mMode)
      {
      default:
      case 1:
      ...
      do
      {
        do
        {
          do
          {
            do
            {
              do
              {
                ...
                {
                  return;
                  i = PLA_AbsListView.this.getChildCount() - 1;
                  k += i;
                }
                while (i < 0);
                ...
                localView = PLA_AbsListView.this.getChildAt(i);
                m = localView.getHeight();
                int n = localView.getTop();
                if (k < PLA_AbsListView.this.mItemCount - 1);
                for (i = this.mExtraScroll; ; i = PLA_AbsListView.this.mListPadding.bottom)
                {
                  PLA_AbsListView.this.smoothScrollBy(m - (j - n) + i, this.mScrollDuration);
                  this.mLastSeenPos = k;
                ...
                i = PLA_AbsListView.this.getChildCount();
              }
              while ((k == this.mBoundPos) || (i <= 1) || (k + i >= PLA_AbsListView.this.mItemCount));
              i = k + 1;
              ...
              localView = PLA_AbsListView.this.getChildAt(1);
              j = localView.getHeight();
              k = localView.getTop();
              m = this.mExtraScroll;
              if (i < this.mBoundPos)
              {
                PLA_AbsListView.this.smoothScrollBy(Math.max(0, j + k - m), this.mScrollDuration);
                this.mLastSeenPos = i;
                PLA_AbsListView.this.post(this);
                return;
              }
            ...
            while (k <= m);
            PLA_AbsListView.this.smoothScrollBy(k - m, this.mScrollDuration);
            return;
            if (k == this.mLastSeenPos)
            {
              PLA_AbsListView.this.post(this);
            ...
            localView = PLA_AbsListView.this.getChildAt(0);
          }
          while (localView == null);
          j = localView.getTop();
          if (k > 0);
          for (i = this.mExtraScroll; ; i = PLA_AbsListView.this.mListPadding.top)
          {
            PLA_AbsListView.this.smoothScrollBy(j - i, this.mScrollDuration);
            this.mLastSeenPos = k;
            if (k <= this.mTargetPos)
              break;
            PLA_AbsListView.this.post(this);
          ...
          m = PLA_AbsListView.this.getChildCount() - 2;
        }
        while (m < 0);
        i = k + m;
        if (i == this.mLastSeenPos)
        ...
        View localView = PLA_AbsListView.this.getChildAt(m);
        k = localView.getHeight();
        int m = localView.getTop();
        this.mLastSeenPos = i;
        if (i > this.mBoundPos)
        {
          PLA_AbsListView.this.smoothScrollBy(-(j - m - this.mExtraScroll), this.mScrollDuration);
          PLA_AbsListView.this.post(this);
          return;
        }
      ...
      while (i <= j);
      PLA_AbsListView.this.smoothScrollBy(-(i - j), this.mScrollDuration);
    }

    void start(int paramInt)
    {
      int i = PLA_AbsListView.this.mFirstPosition;
      int j = PLA_AbsListView.this.getChildCount() + i - 1;
      if (paramInt <= i)
      {
        i = i - paramInt + 1;
        this.mMode = 2;
    ...
    void start(int paramInt1, int paramInt2)
    {
      ...
      {
        return;
        i = PLA_AbsListView.this.mFirstPosition;
        j = PLA_AbsListView.this.getChildCount() + i - 1;
        if (paramInt1 > i)
          break;
        j -= paramInt2;
      }
  ...
  class RecycleBin
  {
    ...
    private void pruneScrapViews()
    {
      ...
      while (true)
      {
        ...
        PLA_AbsListView.this.removeDetachedView((View)localArrayList.remove(j), false);
        k += 1;
        j -= 1;
      }
    }
    ...
    void addScrapView(View paramView)
    {
      ...
      while (true)
      {
        ...
        {
          if (i != -2)
            PLA_AbsListView.this.removeDetachedView(paramView, false);
        }
        else
        {
          if (this.mViewTypeCount == 1)
          {
            PLA_AbsListView.this.dispatchFinishTemporaryDetach(paramView);
    ...
    void clear()
    {
      ...
      if (this.mViewTypeCount == 1)
      {
        ...
        while (true)
        {
          if (i >= j)
            return;
          PLA_AbsListView.this.removeDetachedView((View)localArrayList.remove(j - 1 - i), false);
          i += 1;
        }
      }
      ...
      while (true)
      {
        ...
        PLA_AbsListView.this.removeDetachedView((View)localArrayList.remove(m - 1 - j), false);
        j += 1;
      }
    }
    ...
    void fillActiveViews(int paramInt1, int paramInt2)
    {
      ...
      while (true)
      {
        if (paramInt2 >= paramInt1)
          return;
        View localView = PLA_AbsListView.this.getChildAt(paramInt2);
    ...
    View getScrapView(int paramInt)
    {
      ...
      do
      {
        do
        {
          ...
          {
            return localObject1;
            paramInt = PLA_AbsListView.this.mAdapter.getItemViewType(paramInt);
            localObject1 = localObject2;
          }
          while (paramInt < 0);
          localObject1 = localObject2;
    ...
    void scrapActiveViews()
    {
      ...
      if (localView != null)
      {
        m = ((PLA_AbsListView.LayoutParams)localView.getLayoutParams()).viewType;
        arrayOfView[k] = null;
        ...
        if (m != -2)
        {
          PLA_AbsListView.this.removeDetachedView(localView, false);
          localObject2 = localObject1;
        }
      }
      while (true)
      {
        ...
        PLA_AbsListView.this.dispatchFinishTemporaryDetach(localView);
 ...
 * Qualified Name:     com.huewu.pla.lib.internal.PLA_AbsListView
