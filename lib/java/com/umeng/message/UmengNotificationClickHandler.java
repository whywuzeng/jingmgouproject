  ...
  implements UHandler
{
  ...
  public void autoUpdate(Context paramContext, UMessage paramUMessage)
  {
    try
    {
      paramUMessage = MsgLogStore.getInstance(paramContext).getMsgConfigInfo_UpdateResponse();
      ...
      if (paramUMessage != null)
        localMethod.invoke(localClass1, new Object[] { paramContext, localClass2.cast(paramUMessage) });
      return;
    }
  ...
  public void dismissNotification(Context paramContext, UMessage paramUMessage)
  {
  }
  ...
  public void handleMessage(Context paramContext, UMessage paramUMessage)
  {
    try
    {
      if (!paramUMessage.clickOrDismiss)
      {
        dismissNotification(paramContext, paramUMessage);
        return;
      }
      if ((TextUtils.equals("autoupdate", paramUMessage.display_type)) && (PushAgent.getInstance(paramContext).isIncludesUmengUpdateSDK()))
      {
        autoUpdate(paramContext, paramUMessage);
        return;
      }
    }
    catch (Exception paramContext)
