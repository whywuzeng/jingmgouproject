...
public class AbMultiColumnBaseListView extends AbMultiColumnAbsListView
{
  ...
  public AbMultiColumnBaseListView(Context paramContext)
  {
    super(paramContext);
    init(null);
  }

  public AbMultiColumnBaseListView(Context paramContext, AttributeSet paramAttributeSet)
  {
  ...
  private void init(AttributeSet paramAttributeSet)
  {
    ...
    while (true)
    {
      if (i >= getColumnNumber())
      {
        ...
        return;
        if ((this.mFrameRect.width() > this.mFrameRect.height()) && (3 != -1))
  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    while (true)
    {
      ...
      this.mColumns[paramInt1].mColumnLeft = (this.mListPadding.left + this.mColumnPaddingLeft + this.columnWidth * paramInt1);
      paramInt1 += 1;
    }
  }
  ...
  private class Column
  {
    ...
    {
      int i = -2147483648;
      int m = AbMultiColumnBaseListView.this.getChildCount();
      int k = 0;
      int j;
      if (k >= m)
      {
      ...
      View localView = AbMultiColumnBaseListView.this.getChildAt(k);
      if ((localView.getLeft() != this.mColumnLeft) && (!AbMultiColumnBaseListView.this.isFixedView(localView)));
      while (true)
      {
        k += 1;
        break;
    ...
    public void offsetTopAndBottom(int paramInt)
    {
      if (paramInt == 0)
        return;
      int j = AbMultiColumnBaseListView.this.getChildCount();
      int i = 0;
      label15: View localView;
      if (i < j)
      {
        localView = AbMultiColumnBaseListView.this.getChildAt(i);
        if ((localView.getLeft() == this.mColumnLeft) || (AbMultiColumnBaseListView.this.isFixedView(localView)))
          break label61;
      }
      while (true)
  ...
  private class FixedColumn extends AbMultiColumnBaseListView.Column
  {
    public FixedColumn()
    {
      super(2147483647);
 ...
 * Qualified Name:     com.ab.view.pullview.AbMultiColumnBaseListView
